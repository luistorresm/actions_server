...
invoices = record.browse([352, 336, 326, 310, 288, 250, 244, 238, 144, 10, 28832, 4, 10546, 6872, 5786, 5784, 16624, 12894, 28494, 28709])
invoice_not_updated = record.browse([640, 638, 630, 590, 586, 584, 556, 554, 27544, 490, 452, 396, 394]) # CFDI con 4 decimales y Odoo solo 2
# 3.2 (490, 452, 396, 394)
for inv in (invoices - invoice_not_updated):
  ...
  for conc in cfdi.Conceptos.Concepto:
    ...
    lines = inv.invoice_line_ids.filtered(lambda l: l.product_id.default_code == no_id and l.quantity == float(conc.get('cantidad')))
    for line in lines:
  ...
  if inv.amount_total != total_cfdi and inv.id not in [488, 440, 384, 354]: # Excepciones por minimos
